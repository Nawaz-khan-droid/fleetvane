...
# Find JDK directory
$JDK_DIR = Get-ChildItem -Path "$PSScriptRoot\.jdk" -Recurse -Filter "java.exe" -ErrorAction SilentlyContinue | Select-Object -First 1 | ForEach-Object { $_.Directory.Parent.FullName }
if (-not $JDK_DIR) {
...
$MAVEN_DIR = Get-ChildItem -Path "$PSScriptRoot\backend\maven_extracted" -Recurse -Filter "mvn.cmd" -ErrorAction SilentlyContinue | Select-Object -First 1 | ForEach-Object { $_.Directory.Parent.FullName }
if (-not $MAVEN_DIR) {
