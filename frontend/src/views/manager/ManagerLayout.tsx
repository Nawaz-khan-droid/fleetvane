...
// Bottom nav: max 5 items — Profile accessed via sidebar/hamburger only
  ...
  return (
    <div className="min-h-screen flex bg-slate-50 dark:bg-slate-950 overflow-x-hidden">
      ...
      <div className="flex-1 flex flex-col min-h-screen min-w-0">
        {/* Top bar */}
        <header
          className={`${theme.nav.topBar} flex items-center justify-between px-4 lg:px-8 h-16 bg-white/80 dark:bg-slate-900/80 backdrop-blur-md border-b border-slate-200/50 dark:border-slate-800/50 shadow-sm`}
        >
          ...
          <div className="flex items-center gap-3 min-w-0">
            ...
            <div className="hidden md:flex">
              <LiveGreeting name={authState.user?.name || 'Manager'} />
            </div>
          </div>

          {/* Right: mobile=avatar only, sm+=theme+bell, md+=user info+badge */}
          <div className="flex items-center gap-2 sm:gap-3 shrink-0">
            ...
            <div className="hidden md:flex items-center gap-3">
              ...
              <Badge
                className={`${theme.status.badge} ${theme.brand.primaryText} bg-emerald-50 dark:bg-emerald-950 border-emerald-200 dark:border-emerald-800`}
              >
                Manager
              </Badge>
            </div>
            ...
            <button
              ...
              aria-label="Profile"
            >
              {authState.user?.name?.charAt(0)?.toUpperCase() || 'M'}
            </button>
          </div>
        </header>

        {/* Content */}
        <main className={`${theme.layout.contentArea} flex-1 pt-20 lg:pt-24 px-4 lg:px-8 pb-24 lg:pb-8`}
        >
          <motion.div
            ...
            {mobileNavItems.map((item) => {
              ...
              return (
                <button
                  key={item.path}
                  onClick={() => navigate(item.path)}
                  className={`flex flex-col items-center justify-center gap-0.5 px-2 h-full transition-colors ${
                    active
